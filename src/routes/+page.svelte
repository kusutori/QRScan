...
<div class="p-6 max-w-5xl mx-auto w-full pt-12 md:pt-6 h-full flex flex-col">
  <div class="flex items-center gap-4 mb-8 shrink-0">
    <div class="p-3 bg-white/50 dark:bg-slate-800/50 backdrop-blur-md text-indigo-600 dark:text-indigo-400 rounded-xl shadow-sm border border-white/60 dark:border-slate-700/60 transition-colors">
      <QrCode size={24} />
    </div>
    <h1 class="text-2xl font-bold text-slate-800 dark:text-white tracking-tight transition-colors">生成二维码</h1>
  </div>
  ...
  <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-8 min-h-0 items-start perspective-1000 relative transition-all duration-1000 ease-in-out">
    ...
    ">
      <div class="bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl p-8 rounded-[2rem] shadow-2xl border border-white/50 dark:border-slate-800/50 transition-all duration-500 hover:shadow-indigo-100/50 dark:hover:shadow-indigo-900/20 h-full">
        <label for="url-input" class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-4 ml-1 transition-colors">链接地址</label>
        <div class="relative group mb-8">
            <div class="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none text-slate-400 dark:text-slate-500 group-focus-within:text-indigo-500 dark:group-focus-within:text-indigo-400 transition-colors duration-300">
              <Link size={20} />
            </div>
            <input
              id="url-input"
              ...
              class="w-full pl-14 pr-6 py-5 bg-slate-50/50 dark:bg-slate-950/50 border-2 border-slate-100/80 dark:border-slate-800 rounded-2xl outline-none focus:border-indigo-500 dark:focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-900 focus:shadow-[0_0_0_4px_rgba(99,102,241,0.1)] dark:focus:shadow-[0_0_0_4px_rgba(99,102,241,0.2)] transition-all duration-300 placeholder:text-slate-400 dark:placeholder:text-slate-600 font-medium text-lg text-slate-900 dark:text-slate-100"
            />
        </div>
        ...
        <button
            ...
            class="w-full py-5 rounded-2xl font-bold text-lg text-white shadow-xl shadow-indigo-200/80 dark:shadow-indigo-900/40 flex items-center justify-center gap-2 transition-all duration-300 transform hover:-translate-y-1 active:scale-[0.98] active:translate-y-0 {!url ? 'bg-slate-300 dark:bg-slate-700 cursor-not-allowed shadow-none' : 'bg-indigo-600 hover:bg-indigo-700 hover:shadow-indigo-400/50'}"
        >
            {#if loading}
    ...
    ">
        <div 
            id="qr-card"
            class="
                relative w-full bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-white/60 dark:border-slate-800/60 shadow-2xl flex flex-col overflow-hidden
                transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1)
                rounded-[2.5rem]
                {isGenerated ? 'shadow-indigo-200/40 dark:shadow-indigo-900/20 bg-white/80 dark:bg-slate-900/80 scale-100 opacity-100' : 'shadow-slate-200/40 dark:shadow-slate-900/40 bg-white/30 dark:bg-slate-900/30 scale-[0.98] opacity-80 grayscale-[0.5] dark:grayscale-[0.8]'}
            "
        ...
        >
            ...
            {#if isGenerated}
                <div in:fly={{ y: 50, duration: 600, delay: 300, easing: cubicOut }} class="p-8 pt-0 z-10">
                    <div class="flex items-center gap-3 mb-6 p-4 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl border border-white dark:border-slate-700 shadow-sm group-hover:border-indigo-100 dark:group-hover:border-indigo-900/50 transition-colors">
                        <div class="p-2 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg text-indigo-500 dark:text-indigo-400 shrink-0">
                            <Link size={16} />
                        </div>
                        <div class="flex-1 truncate text-sm text-slate-600 dark:text-slate-300 font-medium select-all">{url}</div>
                        <button 
                            onclick={handleCopyLink} 
                            class="relative group-btn p-2 rounded-xl transition-all duration-300 active:scale-90 {copied ? 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400' : 'text-slate-400 dark:text-slate-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30'}" 
                            title="复制链接"
                        >
                    ...
                    <button 
                        ...
                        class="w-full py-4 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 rounded-2xl font-bold hover:bg-slate-800 dark:hover:bg-white transition-all shadow-lg shadow-slate-300 dark:shadow-white/5 flex items-center justify-center gap-2 active:scale-95 hover:-translate-y-1"
                    >
                        <Download size={20} />
                        下载图片
                    </button>
